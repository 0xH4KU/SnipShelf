...
- Snap assistance now combines native-resolution local pixel edges with [Apple Vision contours](https://developer.apple.com/documentation/vision/vndetectcontoursrequest). Pixel assistance is available immediately, even if Vision is still working. Thin screen/browser borders are no longer dependent solely on the downscaled Vision image.
- The previous contour receives only a small preference. A closer border can win instead of the old contour remaining locked. **Tab** cycles nearby alternatives; **Option** temporarily bypasses snapping. Detection is still edge assistance, not guaranteed semantic object segmentation.
...
Adaptive steadiness remains available in the wand menu (default 55%; zero disables it). Snap distance defaults to 10 screen points and can be set from 4–24 pt. Choices persist. Vision runs locally in the background on a 1536-pixel working image; export retains the original pixels. Local pixel searches have a bounded sampling grid when very far zoomed out. Low contrast, dense texture, or several nearby edges can still require manual choice.
