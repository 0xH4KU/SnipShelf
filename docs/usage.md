...
- Card buttons use the same opening transition: the card and the actual window grow together from the card's bounds over about 0.3 seconds as the window content fades in. Without a saved position, references opened by clicking or using a shortcut appear beside the Shelf and cascade into the available space, leaving it uncovered. If neither side has enough room, placement uses the space above or below. Single-image references show their thumbnail immediately while the original loads. **Reduce Motion** replaces the expanding/returning motion with a short crossfade.
...
- With two or more references open, each floating window shows **Close All · N** on the right of its title bar. Click it to close all references even while the Shelf is tucked away. **Close All Reference Windows** in the Shelf or menu bar options and **Option–Command–W** also close all references, including hidden ones. Each window's size and position are remembered; the Shelf, preview and original clips stay available.
