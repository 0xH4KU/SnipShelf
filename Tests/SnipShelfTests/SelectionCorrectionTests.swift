...
final class SelectionCorrectionTests: XCTestCase {
    ...
    @MainActor func testSubjectMaskFallbackAndExistingPreferencePreserveTheSelection() async throws {
        ...
        preferences.set(false, forKey: "labFluidDrawing")
        preferences.set(false, forKey: "labShowRemovedAreas")
        ...
        XCTAssertFalse(model.fluidDrawing)
        XCTAssertFalse(model.showRemovedAreas)
        model.fluidDrawing = true; model.showRemovedAreas = true
        let replacement = CanvasModel(image: source, isScreen: false, preferences: preferences, complete: { _ in }, cancel: {})
        XCTAssertTrue(replacement.fluidDrawing, "Shared settings must take precedence over old Lab preferences")
        XCTAssertTrue(replacement.showRemovedAreas)
