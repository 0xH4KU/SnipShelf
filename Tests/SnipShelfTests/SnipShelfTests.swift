...
final class SnipShelfTests: XCTestCase {
    ...
    @MainActor func testShelfMoveAndDockState() async throws {
        ...
        XCTAssertEqual(shelf.panel.frame.width, ShelfWindow.tabSize.width, accuracy: 1)
        XCTAssertEqual(shelf.panel.frame.height, ShelfWindow.tabSize.height, accuracy: 1)
        shelf.expand()
        try await Task.sleep(for: .milliseconds(300))
        XCTAssertFalse(shelf.collapsed)
